...
class Path implements Rule
{
    ...
    public function equal(string $host): Path
    {
        $rule = new Rule\Path\Equal($host);
        if (!$this->byEqual) {
            $this->byEqual = $rule;
    ...
    public function match(string $expression): Path
    {
        $rule = new Rule\Path\Match($expression);
        if (!$this->byMatch) {
            $this->byMatch = $rule;
    ...
    public function pattern(string $pattern): Path
    {
        $rule = new Rule\Path\Pattern($pattern);
        if (!$this->byPattern) {
            $this->byPattern = $rule;
